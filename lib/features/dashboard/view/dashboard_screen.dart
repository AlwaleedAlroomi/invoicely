...
class DashboardScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final defaultCurrency = DefaultSettings.getCurrency(ref.read(sharedPreferencesProvider));
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            _buildSummaryRow(context, invoicesAsync, clientsAsync, productsAsync, defaultCurrency),
  ...
  Widget _buildSummaryRow(
    ...
    final isLoading = invoicesAsync.isLoading || clientsAsync.isLoading || productsAsync.isLoading;
    if (isLoading) {
  ...
  Widget _buildRecentInvoices(
          ...
          child: Center(
            child: Column(
              children: [
                ...
                Text('No invoices yet', style: Theme.of(context).textTheme.bodyMedium),
              ],
            ),
          ),
        ),
    ...
    return Column(
      children: recent.take(5).map((inv) => _buildInvoiceTile(context, ref, inv)).toList(),
    );
  }

  Widget _buildInvoiceTile(BuildContext context, WidgetRef ref, InvoiceModel invoice) {
    Color statusColor;
    switch (invoice.status) {
      case InvoiceStatus.paid: statusColor = Colors.green;
      case InvoiceStatus.overdue: statusColor = Colors.red;
      case InvoiceStatus.draft: statusColor = Colors.grey;
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text(client?.name ?? 'Unknown', maxLines: 1, overflow: TextOverflow.ellipsis),
        trailing: Column(
    ...
    return Row(
      children: [
        Expanded(child: _ActionButton(
          ...
          color: Colors.indigo,
          onTap: () async {
            await Navigator.of(context).push(FadeThroughRoute(page: const InvoiceFormScreen()));
            ref.invalidate(allInvoicesProvider);
          },
        )),
        const SizedBox(width: 8),
        Expanded(child: _ActionButton(
          ...
          color: Colors.teal,
          onTap: () async {
            await Navigator.of(context).push(FadeThroughRoute(page: const ClientFormScreen()));
            ref.invalidate(allClientsProvider);
          },
        )),
        const SizedBox(width: 8),
        Expanded(child: _ActionButton(
          ...
          color: Colors.amber,
          onTap: () async {
            await Navigator.of(context).push(FadeThroughRoute(page: const ProductFormPage()));
            ref.invalidate(allProductsProvider);
          },
        )),
      ],
